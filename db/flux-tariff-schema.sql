...
CREATE TABLE victron_tariff_periods (
    ...
    export_rate_pence DECIMAL(6,2) NOT NULL, -- pence per kWh
...
CREATE TABLE victron_grid_setpoints (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(device_id, tariff_period)
);
...
CREATE TABLE victron_energy_tracking (
    ...
    -- Financial calculations (pence)
    import_cost_pence DECIMAL(10,2) DEFAULT 0,
    export_earnings_pence DECIMAL(10,2) DEFAULT 0,
    net_cost_pence DECIMAL(10,2) DEFAULT 0, -- import_cost - export_earnings
...
CREATE TABLE victron_tariff_events (
    ...
    event_type VARCHAR(30) NOT NULL, -- 'period_change', 'setpoint_change', 'soc_target', 'mode_change'
...
-- Insert Octopus Flux tariff rates
...
COMMENT ON TABLE victron_energy_tracking IS 'Detailed energy import/export tracking for financial analysis';

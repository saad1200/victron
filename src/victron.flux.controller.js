...
const LOG_FILE = path.join(__dirname, "../logs/victron-flux-controller.log");

// Controller state
let isRunning = false;
let schedulerInterval = null;
let tariffInterval = null;
...
// Load tariff configuration from database for Octopus Flux product only
async function loadTariffConfig() {
  try {
    // Get Octopus Flux product ID
    const productQuery = `SELECT id FROM products WHERE name = 'Octopus Flux' AND active = true`;
    const productResult = await dbClient.query(productQuery);
    
    if (productResult.rows.length === 0) {
      log('Octopus Flux product not found or not active', 'ERROR');
      return false;
    }
    
    const fluxProductId = productResult.rows[0].id;
    log(`Loading tariff config for Octopus Flux (product_id: ${fluxProductId})`);
    
    // Get tariff periods for Octopus Flux only
    const periodsQuery = `
    ...
    `;
    const periodsResult = await dbClient.query(periodsQuery, [fluxProductId]);
    
    // Get grid setpoints for Octopus Flux only
    ...
    `;
    const setpointsResult = await dbClient.query(setpointsQuery, [DEVICE_ID, fluxProductId]);
...
async function log(message, level = "INFO") {
  ...
  const entry = `[${timestamp}] [FLUX-CONTROLLER] [${level}] ${message}\n`;
  try {
...
// Get current tariff period and configuration from database for Octopus Flux only
  ...
  try {
    ...
    const query = `
      ...
      WHERE vtp.is_active = true AND vgs.is_active = true
        AND vtp.product_id = $2 AND vgs.product_id = $2
        AND (
    ...
    const result = await dbClient.query(query, [currentTime, fluxProductId]);
...
// Start function for the Flux controller
async function start() {
  if (isRunning) {
    log('Flux controller is already running', 'WARN');
    return;
  }
  ...
  try {
    ...
    if (!configLoaded) {
      log('Failed to load tariff configuration', 'ERROR');
      isRunning = false;
      throw new Error('Failed to load tariff configuration');
    }
    
    // Debug: Show loaded TARIFF object
    log(`Loaded TARIFF object with ${Object.keys(TARIFF).length} periods`);
    ...
    tariffInterval = setInterval(async () => {
      if (isRunning) {
        await applyTariffStrategy();
      }
    }, 60 * 1000);
...
// Stop function for the Flux controller
async function stop() {
  if (!isRunning) {
    log('Flux controller is not running', 'WARN');
    return;
  }
  
  log('Stopping Victron Flux Controller...');
  isRunning = false;
  ...
  // Stop setpoint adjuster
  stopSetpointAdjuster();
  
  // End charge session if active
  if (currentChargeSessionId) {
    await endChargeSession('controller_stopped');
  }
  ...
  log('Flux controller stopped successfully');
}
...
if (require.main === module) {
  ...
  start().catch(error => {
    console.error('Failed to start Flux controller:', error.message);
    process.exit(1);
  });
}
